(function () {
  ...
  class GraphView extends View {
      ...
      constructor(id, broker, showPhaseByName) {
          ...
          const svg$$1 = this.divElement.append("svg").attr('version', '1.1')
              .attr("width", "100%")
              .attr("height", "100%");
          ...
          this.selectionHandler = {
              ...
              brokeredNodeSelect: function (locations, selected) {
                  let selection$$1 = graph.nodes
                      .filter(function (n) {
                      return locations.has(nodeToStringKey(n))
                          && (!graph.state.hideDead || n.isLive());
                  });
                  graph.state.selection.select(selection$$1, selected);
          ...
          select("#layout").on("click", partial(this.layoutAction, graph));
          select("#show-all").on("click", partial(this.showAllAction, graph));
          select("#toggle-hide-dead").on("click", partial(this.toggleHideDead, graph));
          select("#hide-unselected").on("click", partial(this.hideUnselectedAction, graph));
          select("#hide-selected").on("click", partial(this.hideSelectedAction, graph));
          select("#zoom-selection").on("click", partial(this.zoomSelectionAction, graph));
          select("#toggle-types").on("click", partial(this.toggleTypesAction, graph));
          // listen for key events
          select(window).on("keydown", function (e) {
          ...
          function zoomed() {
              if (event.shiftKey)
                  return false;
              graph.graphElement.attr("transform", event.transform);
          }
          const zoomSvg = zoom()
              .scaleExtent([0.2, 40])
              .on("zoom", zoomed)
      ...
      measureText(text$$1) {
          ...
          textMeasure.textContent = text$$1;
          return {
      ...
      createGraph(data, rememberedSelection) {
          ...
          data.nodes.forEach(function (n, i) {
              ...
              n.width = alignUp(innerwidth + NODE_INPUT_WIDTH * 2, NODE_INPUT_WIDTH);
      ...
      attachSelection(s) {
          ...
          if (!(s instanceof Set))
              return;
          graph.selectionHandler.clear();
          const selected = graph.nodes.filter((n) => s.has(graph.state.selection.stringKey(n)) && (!graph.state.hideDead || n.isLive()));
      ...
      searchInputAction(searchBar, e) {
          ...
          if (e.keyCode == 13) {
              ...
              const selection$$1 = graph.nodes.filter(function (n, i) {
              ...
              graph.selectionHandler.select(selection$$1, true);
      ...
      svgKeyDown() {
          ...
          if (state.lastKeyDown !== -1)
              return;
          var showSelectionFrontierNodes = function (inEdges, filter, select$$1) {
          ...
          switch (event.keyCode) {
              ...
              case 57:
                  // '1'-'9'
                  showSelectionFrontierNodes(true, (edge, index) => { return index == (event.keyCode - 49); }, false);
                  break;
              case 97:
              case 98:
              case 99:
              ...
              case 105:
                  // 'numpad 1'-'numpad 9'
                  showSelectionFrontierNodes(true, (edge, index) => { return index == (event.keyCode - 97); }, false);
                  break;
              case 67:
                  // 'c'
                  showSelectionFrontierNodes(event.altKey, (edge, index) => { return edge.type == 'control'; }, true);
                  break;
              case 69:
                  // 'e'
                  showSelectionFrontierNodes(event.altKey, (edge, index) => { return edge.type == 'effect'; }, true);
                  break;
              case 79:
                  // 'o'
      ...
      updateGraphVisibility() {
          ...
          const selEdges = graph.visibleEdges.selectAll("path").data(filteredEdges, edgeToStr);
          // remove old links
          ...
          const allNodes = graph.visibleNodes.selectAll("g");
          ...
          newGs.classed("turbonode", function (n) { return true; })
              ...
              const visibleEdges = graph.visibleEdges.selectAll('path');
              ...
              const visibleNodes = graph.visibleNodes.selectAll("g");
              const input = visibleNodes.data(adjInputNodes, nodeToStr)
                  .attr('relToHover', "input");
              const adjOutputNodes = adjOutputEdges.data().map(e => e.target);
              const output = visibleNodes.data(adjOutputNodes, nodeToStr)
          ...
          graph.visibleBubbles = selectAll('circle');
      ...
      viewSelection() {
          ...
          if (hasSelection) {
              graph.viewGraphRegion(minX - NODE_INPUT_WIDTH, minY - 60, maxX + NODE_INPUT_WIDTH, maxY + 60, true);
          }
      }
      viewGraphRegion(minX, minY, maxX, maxY, transition$$1) {
          ...
          const transform$$1 = identity$8.translate(1500, 100).scale(0.75);
          this.svg
  ...
  class ScheduleView extends TextView {
      ...
      elementForBlock(block) {
          ...
          if (block.pred.length)
              schedule_block.appendChild(block_pred);
          ...
          if (block.succ.length)
              schedule_block.appendChild(block_succ);
  ...
  class SequenceView extends TextView {
      ...
      elementForBlock(block) {
          ...
          if (block.predecessors.length > 0)
              sequence_block.appendChild(block_pred);
          ...
          if (block.successors.length > 0)
              sequence_block.appendChild(block_succ);
  ...
  class GraphMultiView extends View {
      ...
      constructor(id, selectionBroker, sourceResolver) {
          ...
          this.graph = new GraphView(id, selectionBroker, (phaseName) => view.displayPhaseByName(phaseName));
  ...
  class Resizer {
      constructor(panes_updated_callback, dead_width) {
          ...
          resizer.resizer_left = select('.resizer-left');
          resizer.resizer_right = select('.resizer-right');
          ...
          let dragResizeLeft = drag()
              .on('drag', function () {
              let x = mouse(this.parentElement)[0];
  ...
  window.onload = function () {
      ...
      function initializeUploadHandlers() {
          ...
          // button #upload.
          select("#upload").on("click", () => document.getElementById("upload-helper").click());
          select("#upload-helper").on("change", function () {
